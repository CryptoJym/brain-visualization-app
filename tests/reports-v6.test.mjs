...
test('old schema-4 save reopens with unknown insights, not invented defaults',()=>{const st=storage();st.setItem(STORAGE_KEY,JSON.stringify({schemaVersion:4,questionnaireVersion:QUESTIONNAIRE_VERSION,answers:SAMPLE_ANSWERS}));const r=readSavedRecord(st);assert.deepEqual(r.insights,{});assert.equal(r.answers.household_incarceration.value,'yes');});
test('unknown insight version is not silently reinterpreted',()=>{const st=storage();st.setItem(STORAGE_KEY,JSON.stringify({schemaVersion:4,questionnaireVersion:QUESTIONNAIRE_VERSION,answers:{},insightsVersion:'future',insights:SAMPLE_INSIGHTS}));assert.throws(()=>readSavedRecord(st),/different present-day questionnaire version/);});
