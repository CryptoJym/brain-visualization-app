...
const target=entries.find(e=>e.account===metadata.verification.account);if(!target||![1,2].includes(target.limit))throw new Error('Unexpected verification grant; nothing changed.');
target.limit=2;metadata.verification.limit=2;
...
writeFileSync('.local-evidence/xai-portraits/access-metadata.json',JSON.stringify(metadata,null,2));console.log(receipt.trim());console.log('Verification workspace allowance is two total attempts, including the first interrupted attempt. Owner grant unchanged.');
