...
export async function generateXaiPortrait(spec,{apiKey,fetchImpl=fetch,timeoutMs=180000}={}){
 if(typeof apiKey!=='string'||!apiKey.trim())throw new PortraitError('image_service_not_configured',503);
 let response;try{response=await fetchImpl('https://api.x.ai/v1/images/generations',{method:'POST',headers:{Authorization:`Bearer ${apiKey}`,'Content-Type':'application/json'},body:JSON.stringify(portraitRequest(spec)),signal:AbortSignal.timeout(timeoutMs),redirect:'error'});}catch{throw new PortraitError('provider_outcome_unknown',502);}
 if(!response.ok){await response.body?.cancel();throw new PortraitError(response.status===401||response.status===403?'provider_access_denied':response.status===429?'provider_rate_limit':'provider_rejected',502);}
 let body;try{body=JSON.parse(await boundedText(response,12*1024*1024));}catch(e){if(e instanceof PortraitError)throw e;throw new PortraitError('invalid_provider_response',502);}
 ...
 let bytes;try{bytes=Uint8Array.from(atob(body.data[0].b64_json.replace(/^data:image\/(?:jpeg|png|webp);base64,/,'')),c=>c.charCodeAt(0));}catch{throw new PortraitError('invalid_image_encoding',502);}
 ...
 return {bytes,contentType:jpeg?'image/jpeg':'image/png',sha256:await sha256(bytes),model:typeof body.model==='string'?body.model:XAI_CONFIG.model,moderation:body.respect_moderation===true||body.data[0].respect_moderation===true?'provider-approved':'provider-returned-image',styleVersion:STYLE_VERSION};
}
