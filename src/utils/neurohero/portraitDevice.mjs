...
export async function getDevicePortrait(asset){const a=normalizePortraitAsset(asset);if(!a)return null;const row=await transact('readonly',s=>s.get(a.id));if(!row?.blob||row.sha256!==a.sha256)return null;return row.blob;}
...
export async function saveDevicePortrait(file,consent=false){
 ...
 const sha256=[...new Uint8Array(await crypto.subtle.digest('SHA-256',await blob.arrayBuffer()))].map(n=>n.toString(16).padStart(2,'0')).join('');const id=crypto.randomUUID();await transact('readwrite',s=>s.put({id,sha256,blob,width,height}));return {kind:'device',id,sha256,width,height};
}
