...
export default function CortexCompass(){
  ...
  const start=()=>{setAnswers({});setInsightAnswers({});setSectionIndex(0);setSelectedRegion(null);setLegacyRecord(null);setDemo(false);setSaved(false);setConsent(false);setSaveOpen(false);setMessage('');navigate('assessment');};
  const sample=()=>{setAnswers(normalizeAnswers(SAMPLE_ANSWERS));setInsightAnswers({...SAMPLE_INSIGHTS});setLegacyRecord(null);setDemo(true);setSaved(false);setSelectedRegion(null);setMessage('');navigate('results');};
  const resume=()=>{try{const record=readSavedRecord(localStorage);if(!record){setHasSaved(false);setMessage('No saved reflection was found on this device.');return;}setAnswers(record.answers);setInsightAnswers(record.insights||{});setLegacyRecord(record.legacyRecord);setDemo(false);setSaved(!record.migrated);setSelectedRegion(null);setMessage(record.legacyRecord?'Earlier answers are preserved. Changed questions need a fresh answer; combined responses were not split automatically.':record.evidenceUpdated?'Opened your saved reflection. The research guide has been updated; your answers and age ranges are unchanged.':'Opened your saved reflection.');navigate('results');}catch(e){setMessage(e.message);}};
  const save=()=>{try{saveRecord(localStorage,answers,legacyRecord,consent,insightAnswers);setSaved(true);setHasSaved(true);setMessage('Saved on this device only—not uploaded to Utlyze or a server.');}catch{setSaved(false);setMessage('Could not save. Check the consent box and browser storage permissions. Existing saved data was not intentionally changed.');}};
  ...
  const exportReflection=()=>{
    const data={schemaVersion:4,questionnaireVersion:QUESTIONNAIRE_VERSION,evidenceVersion:profile.evidenceVersion,answers:normalizeAnswers(answers),insights:normalizeInsights(insightAnswers),insightsVersion:INSIGHT_VERSION,legacyRecord,profile};
  ...
  if(screen==='report')return <CompassReports profile={profile} insightAnswers={insightAnswers} answers={answers} demo={demo} onBack={()=>navigate('results')} onEdit={()=>navigate('insights')}/>;
  if(screen==='evidence')return <main className="cc-shell cc-evidence-screen"><header className="cc-nav">{brand}<button className="cc-secondary" onClick={()=>navigate(evidenceReturn)}>← Back to reflection</button></header><div className="cc-research-intro"><span className="cc-eyebrow">QUESTION → STUDY → ANATOMY</span><h1>Explore what the research actually measured.</h1><p>Use a region button to locate a teaching model. Group associations are not predictions about your brain.</p></div><div className="cc-research-layout"><div className="cc-research-brain"><CortexBrain focus={brainFocus}/><p className="cc-research-model-note">26 teaching regions · geometric boundaries, not a segmented atlas.</p></div><ResearchStudyBrowser questionId={researchQuestion} onQuestionChange={setResearchQuestion} onFocus={focusBrain}/></div><ResearchLessons/><QuestionResearchAudit onExplore={id=>{setResearchQuestion(id);window.scrollTo({top:0});}}/><EvidenceLibrary ids={Object.keys(SOURCES).filter(id=>!REGION_STUDIES[id])}/></main>;
